...
exports.aiDrivenCandidateEngagement = aiDrivenCandidateEngagement;
/**
...
const AiDrivenCandidateEngagementInputSchema = genkit_2.z.object({
    candidateName: genkit_2.z.string().describe('The name of the candidate.'),
    candidateStage: genkit_2.z.string().describe('The current stage of the candidate in the hiring process (e.g., Eligible for Role, AI Video Interview Scheduled, Offer Extended, Rejected, Gauntlet Deadline Approaching).'),
    jobTitle: genkit_2.z.string().describe('The title of the job the candidate is applying for.'),
    companyName: genkit_2.z.string().describe('The name of the company.'),
    recruiterName: genkit_2.z.string().describe('The name of the recruiter.'),
    candidateSkills: genkit_2.z.string().describe('The skills of the candidate.'),
    rejectionReason: genkit_2.z.string().optional().describe('The reason for rejection, if the candidate is in the Rejected stage.'),
});
const AiDrivenCandidateEngagementOutputSchema = genkit_2.z.object({
    emailSubject: genkit_2.z.string().describe('The subject of the email.'),
    emailBody: genkit_2.z.string().describe('The body of the personalized email.'),
});
async function aiDrivenCandidateEngagement(input) {
...
const aiDrivenCandidateEngagementFlow = genkit_1.ai.defineFlow({
