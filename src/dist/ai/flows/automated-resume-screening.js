...
exports.automatedResumeScreening = automatedResumeScreening;
/**
...
const AutomatedResumeScreeningInputSchema = genkit_2.z.object({
    resumeDataUri: genkit_2.z
        .string()
        .describe("The resume to screen, as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'"),
    resumeText: genkit_2.z.string().optional().describe('Extracted text from the resume, if not a direct media type.'),
    skillMappings: genkit_2.z.record(genkit_2.z.string(), genkit_2.z.string()).optional().describe('A map of candidate skills to company-preferred skills.'),
    companyPreferences: genkit_2.z.string().optional().describe('Any specific company preferences for candidates.'),
...
const AutomatedResumeScreeningOutputSchema = genkit_2.z.object({
    extractedInformation: genkit_2.z.object({
        name: genkit_2.z.string().describe("The candidate's full name, or the filename if a name cannot be found."),
        email: genkit_2.z.string().optional().describe("The candidate's email address."),
        phone: genkit_2.z.string().optional().describe("The candidate's phone number."),
        socialUrl: genkit_2.z.string().optional().describe("A social profile URL (e.g., LinkedIn, GitHub) found in the resume."),
        skills: genkit_2.z.array(genkit_2.z.string()).describe('A list of skills extracted from the resume.'),
        experience: genkit_2.z.string().describe("A summary of the candidate's experience."),
        narrative: genkit_2.z.string().describe("A 2-3 sentence narrative summary of the candidate's profile."),
        inferredSkills: genkit_2.z.array(genkit_2.z.string()).describe("A list of 2-3 inferred soft skills or related technical skills."),
    }),
    candidateScore: genkit_2.z.number().describe("A score representing the candidate's suitability for the role, between 0 and 100."),
    reasoning: genkit_2.z.string().describe('Explanation of how the candidate score was derived, incorporating skill mappings and company preferences.'),
});
async function automatedResumeScreening(input) {
...
const automatedResumeScreeningFlow = genkit_1.ai.defineFlow({
