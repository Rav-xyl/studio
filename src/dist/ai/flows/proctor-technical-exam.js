...
exports.proctorTechnicalExam = proctorTechnicalExam;
/**
...
const ProctorTechnicalExamInputSchema = genkit_2.z.object({
    question: genkit_2.z.string().describe('The technical question that was asked.'),
    candidateAnswer: genkit_2.z.string().describe("The candidate's full written answer to the question."),
    proctoringLog: genkit_2.z.array(genkit_2.z.string()).describe("A log of proctoring events, such as 'Candidate switched tabs'."),
    ambientAudioTranscript: genkit_2.z.string().describe("A transcript of ambient audio captured during the exam, to detect other voices."),
});
const ProctorTechnicalExamOutputSchema = genkit_2.z.object({
    evaluation: genkit_2.z.string().describe("A concise evaluation of the candidate's written answer, focusing on technical correctness, clarity, and depth."),
    score: genkit_2.z.number().describe('A score from 0 to 100 for the response. A score below 70 is a failure.'),
    proctoringSummary: genkit_2.z.string().describe("A summary of the proctoring analysis, noting any suspicious events."),
    isPass: genkit_2.z.boolean().describe("Whether the candidate passed the technical round (score >= 70)."),
});
async function proctorTechnicalExam(input) {
...
const proctorTechnicalExamFlow = genkit_1.ai.defineFlow({
