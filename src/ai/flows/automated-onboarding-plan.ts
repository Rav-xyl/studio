...
const GenerateOnboardingPlanInputSchema = z.object({
  candidateName: z.string().describe("The new hire's name."),
  roleTitle: z.string().describe("The title of the role the candidate was hired for."),
  roleResponsibilities: z.string().describe("The key responsibilities of the role."),
  candidateStrengths: z.array(z.string()).describe("The candidate's identified strengths from resume analysis and interviews."),
  companyCulture: z.string().describe("A brief description of the company's culture (e.g., 'fast-paced startup', 'structured enterprise')."),
});
...
const GenerateOnboardingPlanOutputSchema = z.object({
  performanceForecast: z.string().describe("A 2-3 sentence prediction of the new hire's long-term success, highlighting key areas for support."),
  onboardingPlan: z.object({
    days30: z.array(z.string()).describe("A list of key goals and activities for the first 30 days."),
    days60: z.array(z.string()).describe("A list of key goals and activities for the next 30 days (days 31-60)."),
    days90: z.array(z.string()).describe("A list of key goals and activities for the final 30 days of the onboarding period (days 61-90)."),
  }),
});
